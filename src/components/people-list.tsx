...
export default function PeopleList({ loading = false, peopleList = [], error = '', onDelete }: PeopleListProps) {
    const hasPeople = peopleList.length > 0
    return (
        <>
            {loading && <div>{'Chargement des données en cours'}</div>}
            <p>{error}</p>
            <div>{hasPeople && <PeopleGrid people={peopleList} onDelete={onDelete} />}</div>
        </>
    )
}
